...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (orgType !== 'academy' || !academyId) return;
    const supabaseAny = supabase as any;

    (async () => {
      // Total players across all squads in this academy
      // TODO: join academies -> clubs -> teams -> players once team relations are confirmed
      const { count: playerCount } = await supabaseAny
        .from('players')
        .select('id', { count: 'exact', head: true });

      // Open injury records
      const { count: injuryCount } = await supabaseAny
        .from('injury_record')
        .select('id', { count: 'exact', head: true })
        .eq('is_resolved', false);

      // Active trial prospects (pipeline_stage != 'rejected' | 'signed' | 'released')
      const { count: prospectCount } = await supabaseAny
        .from('prospect')
        .select('id', { count: 'exact', head: true })
        .eq('academy_id', academyId)
        .not('pipeline_stage', 'in', '("rejected","signed","released")');

      // Upcoming fixtures this week (team_events synced from Football Central)
      const today = new Date();
      const weekEnd = new Date(today);
      weekEnd.setDate(today.getDate() + 7);
      const { count: fixtureCount } = await supabaseAny
        .from('team_events')
        .select('id', { count: 'exact', head: true })
  ...
  useEffect(() => {
    ...
    (async () => {
      const [{ count: p }, { count: m }] = await Promise.all([
        supabaseAny.from('players').select('id', { count: 'exact', head: true }),
        supabaseAny.from('matches').select('id', { count: 'exact', head: true }),
      ]);
      setPlayerCount(p ?? 0);
      setMatchCount(m  ?? 0);
    })();
