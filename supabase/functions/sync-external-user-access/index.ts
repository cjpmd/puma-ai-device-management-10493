...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!user.email) {
      ...
      return new Response(JSON.stringify({ success: true, message: "User access sync completed", results }), {
    ...
    const { data: extProfile } = await externalSupabase.from("profiles").select("id, user_id, email").eq("email", user.email).maybeSingle();
    if (extProfile) {
    ...
    if (!externalUserId) {
      try {
        ...
        const match = usersList?.users?.find((candidate: { email?: string }) => candidate.email === user.email);
        if (match) externalUserId = match.id;
      } catch (error) {
        console.warn("External admin.listUsers not available:", error instanceof Error ? error.message : error);
      }
    }

    if (!externalUserId) {
      console.log(`No external user found for ${user.email}`);
      return new Response(JSON.stringify({ success: true, message: "No external user found", results }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    for (const membership of teamMemberships) {
      const { data: localTeam } = await localSupabase.from("teams").select("id").eq("external_id", membership.team_id).maybeSingle();
    ...
    for (const membership of clubMemberships) {
      const { data: localClub } = await localSupabase.from("clubs").select("id").eq("external_id", membership.club_id).maybeSingle();
